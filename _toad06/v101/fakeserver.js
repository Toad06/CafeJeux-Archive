function CJGame_Action(data) {
	...
	if(isInit) {
		if(player === 1) {
			...
			CJGame_PlayData.init = data;
		} else {
			data = CJGame_PlayData.init;
		}
	}
	return data;
}
...
function CJGame_SetColors(data) {
	...
	var col1 = data[1];
	var col2 = data[2];
	// "col1" correspond toujours à la couleur du joueur actif, "col2" à celle de son adversaire.
...
function CJGame_SetInfos(data) {
	// TODO/FIXME: Les scores affichés ne doivent pas être inversés à chaque tour, problème similaire à celui des couleurs.
	var player = data[0];
	var html = data[1];
...
function Magmax_Init(data) {
	...
	if(CJGame_PlayData.seeds === null) {
		var seed2 = seed ^ Math.floor(Math.random() * 1000);
...
function Miniwar_Place(data) {
	...
	if(isFinite(special)) {
		...
		if(special >= 0 && special <= 5) {
			...
			// Génère différents types de cosmos dans une même partie (hors ninja et mage qui n'ont pas d'armes).
			data[2] = ~~(Math.random() * 4);
		}
	}
}
